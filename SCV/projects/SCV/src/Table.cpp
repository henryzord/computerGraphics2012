...
void Table::processChar(const unsigned int &character, const int &modifier) 
{
	static Kernel *kernel = Kernel::getInstance();

	Panel::processChar(character, modifier);
}
...
void Table::processKey(const scv::KeyEvent &evt) {
   ...
   if (/*!evt.special &&*/ evt.keycode == GLFW_KEY_TAB && evt.state == KeyEvent::PRESS) {
      for (int i = 0; i < _nRows ; i++ ) {
         for (int j = 0; j <  _nColumns; j++ ) {
