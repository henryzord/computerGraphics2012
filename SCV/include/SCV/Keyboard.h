...
public:
   ...
   * \ingroup internal
   ...
   public:
      KeyboardControl(int key, int modifier/*, bool special*/) {
		  this->key = key; this->modifier = modifier; /*this->special = special;*/
      }
      bool operator==(const KeyboardControl& rhs) const {
         return (key == rhs.key && /*special == rhs.special &&*/ modifier == rhs.modifier)? true : false;
      }
      int key;
	  int modifier;
     // bool special;
   ...
   void pop(int key, int modifier/*, bool special*/);
   void push(int key, int modifier/*, bool special*/);
